...
const EmailDashboard = () => {
  ...
  const calculateTotals = (data) => {
    ...
    const storage = data.reduce(
      (acc, curr) => acc + parseInt(curr.totalSize || 0),
      0
    );
  ...
  const prepareStorageData = (data) => {
    ...
    try {
      ...
      return data.map((item) => ({
        ...
        percentage:
          totalSize > 0
            ? ((parseInt(item.totalSize || 0) / totalSize) * 100).toFixed(1)
            : "0",
      }));
    } catch (error) {
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Comprehensive overview of your email categories and usage
            </p>
          </div>
          <button
        ...
        <Card className="shadow-lg mt-8">
          <CardHeader className="border-b bg-white">
            <CardTitle className="text-xl text-gray-800 flex items-center gap-2">
              ...
              Category Details: {categoryIcons[selectedCategory]}{" "}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="bg-gradient-to-br from-blue-500 to-blue-600 text-white">
                  <CardContent className="p-6">
                    ...
                    <p className="text-2xl font-bold mt-2">
                      {categoryDetails.timeAnalysis
                        ?.reduce((acc, curr) => acc + curr.emailCount, 0)
                        .toLocaleString()}
                    </p>
                  </CardContent>
                </Card>
                ...
                <Card className="bg-gradient-to-br from-green-500 to-green-600 text-white">
                  <CardContent className="p-6">
                    <h4 className="text-lg font-medium opacity-90">
                      Average Monthly Emails
                    </h4>
                    <p className="text-2xl font-bold mt-2">
                      {Math.round(
                        categoryDetails.timeAnalysis?.reduce(
                          (acc, curr) => acc + curr.emailCount,
                          0
                        ) / (categoryDetails.timeAnalysis?.length || 1)
                      ).toLocaleString()}
                    </p>
                  </CardContent>
                </Card>
